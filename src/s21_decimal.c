...
s21_decimal s21_mul(s21_decimal x, s21_decimal y) {
    ...
    int scale_res = s21_scale_align(&x, &y);
    ...
    if (s21_is_can_div(x, y, &result) == TRUE) {
        ...
        if (s21_is_can_div(x, y, &result) == TRUE) {
            ...
            do {
                ...
                s21_decimal temp_y = y;
                s21_decimal temp_mod = mod;
                s21_set_scale(&temp_y, 0);
                s21_set_scale(&temp_mod, 0);
                if (s21_is_greater(temp_y, temp_mod) == TRUE) {
                    mod = s21_up_scale(mod);
                }
                x = mod;
                s21_decimal temp1 = result;
                s21_set_scale(&temp1, 0);
                temp1 = s21_small_mul(temp1);
                if (temp1.bits[3] != 0) {
                    stop++;
                }
                if (!s21_is_zero(mod) && (s21_get_decimal_count(result) == 29 ||
                                            s21_get_scale(result) == 28) || stop) {
                    s21_decimal five = {{5, 0, 0, 0}, s21_NORMAL_VALUE};
                    s21_decimal one = {{1, 0, 0, 0}, s21_NORMAL_VALUE};
                    s21_decimal_norm(&mod);
                    s21_set_scale(&five, s21_get_scale(temp));
                    if (s21_is_greater(temp, five) == TRUE) {
                        s21_set_scale(&one, s21_get_scale(result));
                        result = s21_add(result, one);
                    } else if (s21_get_scale(result) < 28 && s21_is_equal(temp, five) == TRUE) {
            ...
            } while (!s21_is_zero(mod) && (s21_get_decimal_count(result) <= 28 && s21_get_scale(result) < 28)
                && !stop);
...
int s21_compare(s21_decimal left, s21_decimal right) {
    int result = 0;
    if (left.bits[0] == 0 && left.bits[1] == 0 && left.bits[2] == 0)
      left.bits[3] = 0;
    if (right.bits[0] == 0 && right.bits[1] == 0 && right.bits[2] == 0)
      right.bits[3] = 0;
            ...
            for (int i = 95; i >= 0; i--) {
                if (s21_get_bit(left, i) != s21_get_bit(right, i)) {
                    if (leftsign > 0 && rightsign > 0)
                        result = s21_get_bit(right, i) - s21_get_bit(left, i);
                    else
    ...
    if (result && value && *value != 0) {
        ...
        if (len < 35) {
            ...
            for (int i = 0; i < exp; i++) {
                ...
                *result = s21_add(*result, temp);
                if (i != exp - 1) *result = s21_small_mul(*result);
            }
            int scale = 0;
            if (exp != len - 1) {
...
int s21_from_decimal_to_string(s21_decimal decimal, char *string) {
    ...
    *p = 0;
    ...
    while (!s21_is_zero(temp) || (p - string) > 0) {
        ...
        *p = '0' + mod.bits[0] % 10;
        ...
        if (scale && (p - string) == point && point > 0) {
            *p = '.';
            p--;
            point = -1;
        }
